...
// Get parent items for patient
app.get('/api/patients/:patientId/parent-items', async (req, res) => {
  try {
    const { patientId } = req.params
    const db = getDb()
...
// Create parent item for patient
app.post('/api/patients/:patientId/parent-items', async (req, res) => {
  try {
    const { patientId } = req.params
    ...
    if (!patient[0].clinicId) {
      return res.status(400).json({ error: 'Paciente sem clínica vinculada' })
    }
...
// Update parent item
app.put('/api/patients/:patientId/parent-items/:id', async (req, res) => {
  try {
    const { patientId, id } = req.params
    const { updatedByUserId, ...updateData } = req.body || {}
